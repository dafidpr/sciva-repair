                    ...
                    <div class="col-md-3 col-sm-6" style="margin-bottom: 10px">
                        @foreach($permission as $singlePermission)
                            <div class="form-check d-block">
                                <input type="checkbox" class="uid mr-1 form-check-input" id="uid-{{$idx . '-' . $loop->iteration }}" name="permission[]" value="{{ $singlePermission['name'] }}" {{ $role->hasPermissionTo($singlePermission['name']) ? "checked" : "" }} /><label for="uid-{{ $idx . '-' . $loop->iteration }}" class="mb-0"> {{ $singlePermission['label'] }}</label>
                            </div>
                        @endforeach
                    </div>
                @endforeach
